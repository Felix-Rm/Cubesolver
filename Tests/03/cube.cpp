...
void rotateZ(tile cube[3][3][3],short dir){
  rotateFront(cube,1-dir);
  rotateBack(cube,dir);

  for (int i = 0; i < 3; i++) {
    tile temp;
    temp                                                              = rotateTileZ(cube[rotateCoords[0+dir][i+0][0]][1][rotateCoords[0+dir][i+0][1]]);
    cube[rotateCoords[0+dir][i+0][0]][1][rotateCoords[0+dir][i+0][1]] = rotateTileZ(cube[rotateCoords[0+dir][i+1][0]][1][rotateCoords[0+dir][i+1][1]]);
    cube[rotateCoords[0+dir][i+1][0]][1][rotateCoords[0+dir][i+1][1]] = temp;

    temp                                                              = rotateTileZ(cube[rotateCoords[0+dir][i+4][0]][1][rotateCoords[0+dir][i+4][1]]);
    cube[rotateCoords[0+dir][i+4][0]][1][rotateCoords[0+dir][i+4][1]] = rotateTileZ(cube[rotateCoords[0+dir][i+5][0]][1][rotateCoords[0+dir][i+5][1]]);
    cube[rotateCoords[0+dir][i+5][0]][1][rotateCoords[0+dir][i+5][1]] = temp;
  }
}

void rotateX(tile cube[3][3][3],short dir){
  rotateLeft(cube,dir);
  rotateRight(cube,1-dir);

  for (int i = 0; i < 3; i++) {
    tile temp;
    temp                                                              = rotateTileX(cube[rotateCoords[1-dir][i+0][0]][rotateCoords[1-dir][i+0][1]][1]);
    cube[rotateCoords[1-dir][i+0][0]][rotateCoords[1-dir][i+0][1]][1] = rotateTileX(cube[rotateCoords[1-dir][i+1][0]][rotateCoords[1-dir][i+1][1]][1]);
    cube[rotateCoords[1-dir][i+1][0]][rotateCoords[1-dir][i+1][1]][1] = temp;

    temp                                                              = rotateTileX(cube[rotateCoords[1-dir][i+4][0]][rotateCoords[1-dir][i+4][1]][1]);
    cube[rotateCoords[1-dir][i+4][0]][rotateCoords[1-dir][i+4][1]][1] = rotateTileX(cube[rotateCoords[1-dir][i+5][0]][rotateCoords[1-dir][i+5][1]][1]);
    cube[rotateCoords[1-dir][i+5][0]][rotateCoords[1-dir][i+5][1]][1] = temp;
  }
}

void executeCommand(char step,tile cube[3][3][3],bool draw){
...
string relativeCoords(string in1,string in2){
  string out = "";

  if(to_string(in1[0] - in2[0]).length() < 2){
    out += '+';
  }
  out += to_string(in1[0] - in2[0]);

  if(to_string(in1[1] - in2[1]).length() < 2){
    out += '+';
  }
  out += to_string(in1[1] - in2[1]);

  if(to_string(in1[2] - in2[2]).length() < 2){
    out += '+';
  }
  out += to_string(in1[2] - in2[2]);
